...
class GoogleFlightsBrowserScraper:
    ...
    def search(
        ...
        try:
            ...
            return self._search_with_fli(origin, destination, departure_date, return_date, max_results)
    ...
    def _search_with_fli(self, origin, destination, dep_date, ret_date, max_results):
        ...
        from fli.models import FlightSearchFilters, FlightSegment, PassengerInfo, Airport
        from fli.search import SearchFlights
        from fli.core.builders import build_flight_segments
        
        try:
            # Build airport objects
            origin_airport = Airport.from_code(origin) if len(origin) == 3 else Airport.lookup(origin)
            dest_airport = Airport.from_code(destination) if len(destination) == 3 else Airport.lookup(destination)
            ...
            return {"flights": flights[:max_results], "total": len(flights), "error": None}
    ...
    def _result_to_dict(self, result) -> Dict:
        ...
        flight_data = {
            "airline": result.airline.name if hasattr(result.airline, 'name') else str(result.airline),
        ...
        if result.legs:
            leg = result.legs[0]
            flight_data["departure_time"] = leg.departure_datetime.strftime("%H:%M") if hasattr(leg, 'departure_datetime') and leg.departure_datetime else ""
            flight_data["arrival_time"] = leg.arrival_datetime.strftime("%H:%M") if hasattr(leg, 'arrival_datetime') and leg.arrival_datetime else ""
