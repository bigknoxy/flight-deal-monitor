...
class FLIClient:
    ...
    def _to_dict(self, result) -> Dict:
        ...
        segment = {
            "flight": {"number": leg.flight_number} if leg else {}
        }
        ...
        if result.booking_token:
            ...
            if actual_token:
                ...
                clean_token = actual_token.replace('"', '').replace('+', '').replace('/', '').replace('=', '')
                booking_url = f"https://www.google.com/travel/flights?q={clean_token[:100]}"
