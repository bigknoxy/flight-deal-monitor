...
class FlightDeal(SQLModel, table=True):
    ...
    expired_at: datetime | None = Field(default=None, description="24h after seen_at")
...
class AlertHistory(SQLModel, table=True):
    ...
    sent_at: datetime = Field(default_factory=lambda: datetime.now(UTC), description="When alert was sent")
    telegram_message_id: str | None = Field(default=None, description="Telegram message ID")
...
class ScheduledJob(SQLModel, table=True):
    ...
    Note: This is a legacy duplicate kept for backward compatibility.
    The canonical definition is in app.models.job.ScheduledJob.
    """
